...
contract DDRMCore is IERC721Full, Ownable {
    ...
    constructor(IERC20 token) public {
        require(address(token) != address(0), "zero address specified as a token contract");
    ...
    */
     ...
     */
    function transferFrom(address from, address to, uint256 tokenId) public canSpend(tokenId) {
    ...
    function buyToken(address recipient, bytes4 assetId) public {
        require(recipient != address(0), "zero address specified as a token recipient");
        require(
        ...
        _mint(recipient, assetId, totalSupply().add(1), block.timestamp.add(30 days));
    }
    ...
    function revokeToken(uint256 tokenId) public onlyOwner {
        require(endTimeOf(tokenId) < block.timestamp, "the specified token is still valid");
     ...
     */
    function setAssetPrice(bytes4 assetId, uint256 price) public onlyOwner {
    ...
    function _clearApproval(uint256 tokenId) private {
        ...
        if (approvedToken.approval != address(0)) approvedToken.approval = address(0);
    }
    ...
    * @param data bytes the transaction metadata
    */
    function _callRecipient(address from, address to, uint256 tokenId, bytes data) private {
        if (to.isContract()) {
            require(
     ...
     */
    function _mint(address to, bytes4 assetId, uint256 tokenId, uint256 endTime) private {
        _addTokenTo(to, tokenId);
