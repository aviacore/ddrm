...
class AssetsList extends Component {
  ...
  render() {
    const { contentList, chooseContent, DDRMCore, chosenContentId, resetContentChoice} = this.props;
    ...
    return contentList.map(({ id, name }) => (
      <div className="el tile" key={id}>
        <div className="el-wrapper" onClick={() => { handleClick(id) }}>
        ...
        <div className="modal-placeholder">
          { chosenContentId === id && <ChosenModal /> }
        </div>
      </div>
    ));
  }
...
const mapStateToProps = ({ contracts: { DDRMCore }, items: { contentList, chosenContentId } }) => ({
  chosenContentId,
  contentList,
  DDRMCore,
});
